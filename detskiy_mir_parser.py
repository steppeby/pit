...
def parser(selected_path = ""):
    ...
    page_lst = []
    if selected_path == '':
        path = 'C:/Users/Jean/PycharmProjects/workspace/venv/detskiy_mir_data.xlsx'  # 37 symbol
    else:
        path = selected_path + 'detskiy_mir_data.xlsx'
    print(path)
    for url in urls:
        ...
        while not parsedData.find_all('p',class_='Wb'):
            ...
            for i in a:
                ...
                desc_url = href['href']
                print('parsing description', desc_url)
                desc = ''
                for retry in range(3):
                    try:
